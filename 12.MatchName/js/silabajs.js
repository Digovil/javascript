...
let encontroTonica = undefined; // (bool)   Indica si se ha encontrado la silaba tónica
...
function posicionSilabas(palabra) {
    ...
    if (!encontroTonica) {
        ...
        if (silaba.numeroSilaba < 2) {
            ...
            let letraFinal = silaba.palabra[silaba.longitudPalabra - 1];
            let letraAnterior = silaba.palabra[silaba.longitudPalabra - 2];
        ...
        switch (c) {
            ...
            case 'Ü':
                if (pos < silaba.longitudPalabra - 1) { // ¿Hay tercera vocal?
                    let siguiente = pal [pos + 1];
                    if (!esConsonante(siguiente)) {
                        let letraAnterior = pal[pos - 1];
                        if (letraAnterior == 'h')
                            pos--;
                        return pos;
                    }
